...
public class RoomNodeGrpahEditor : EditorWindow
{
    ...
    private const float connectingLineArrowSize = 9f;
    ...
    {
        Queue<RoomNodeSO> roomNodeDeletioQueue = new Queue<RoomNodeSO>();

        foreach(RoomNodeSO roomNode in currentRoomNodeGraph.roomNodeList)
        {
            if (roomNode.isSelected && !roomNode.roomNodeType.isEntrance)
            {
                roomNodeDeletioQueue.Enqueue(roomNode);
        ...
        while (roomNodeDeletioQueue.Count > 0)
        {
            RoomNodeSO roomNodeToDelete = roomNodeDeletioQueue.Dequeue();
